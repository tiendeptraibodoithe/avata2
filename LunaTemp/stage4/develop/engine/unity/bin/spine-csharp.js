...
var spine = (() => {
  ...
  var DeformTimeline = class extends CurveTimeline {
    ...
    apply(skeleton, lastTime, time, firedEvents, alpha, blend, direction) {
      ...
      if (!slotAttachment)
        return;
      if (!(slotAttachment instanceof VertexAttachment) || slotAttachment.timelineAttachment != this.attachment && slotAttachment.timelineAttachment.id != this.attachment.id)
        return;
                ...
                while (true) {
                    ...
                    if (line.trim().length === 0) {
                        ...
                        if (region.degrees === 90) {
                            region.u2 = (region.x + region.height) / page.width;
                            region.v2 = (region.y + region.width) / page.height;
                            var tempSwap = region.width;
                            region.width = region.height;
                            region.height = tempSwap;
                        } else {
                            region.u2 = (region.x + region.width) / page.width;
                            region.v2 = (region.y + region.height) / page.height;
                        }
...
if ( TRACE ) { TRACE( "Spine.AtlasAttachmentLoader#newMeshAttachment", this ); }
        ...
        overloads: {
            ...
            "NewRegionAttachment(Skin, string, string, Sequence)": "newRegionAttachment",
            "NewMeshAttachment(Skin, string, string, Sequence)": "newMeshAttachment"
        }
    });
    ...
    Bridge.define("Spine.MeshAttachment", {
        inherits: [spine.MeshAttachment,Spine.IHasTextureRegion],
        alias: [
...
if ( TRACE ) { TRACE( "Spine.Pow#Apply$1", this ); }
    ...
    Bridge.define("Spine.RegionAttachment", {
        inherits: [spine.RegionAttachment,Spine.IHasTextureRegion],
        alias: [
            "Path", "Spine$IHasTextureRegion$Path",
            "Region", "Spine$IHasTextureRegion$Region",
            "R", "Spine$IHasTextureRegion$R",
...
if ( TRACE ) { TRACE( "Spine.RegionAttachment#computeWorldVertices", this ); }
                ...
                offsetX = vertexOffset[6]; // 0
                offsetY = vertexOffset[7]; // 1
                worldVertices[offset] = offsetX * a + offsetY * b + bwx; // bl
                worldVertices[offset + 1] = offsetX * c + offsetY * d + bwy;
                offset += stride;

                offsetX = vertexOffset[0]; // 2
                offsetY = vertexOffset[1]; // 3
                worldVertices[offset] = offsetX * a + offsetY * b + bwx; // ul
                worldVertices[offset + 1] = offsetX * c + offsetY * d + bwy;
                offset += stride;

                offsetX = vertexOffset[2]; // 4
                offsetY = vertexOffset[3]; // 5
                worldVertices[offset] = offsetX * a + offsetY * b + bwx; // ur
                worldVertices[offset + 1] = offsetX * c + offsetY * d + bwy;
                offset += stride;

                offsetX = vertexOffset[4]; // 6
                offsetY = vertexOffset[5]; // 7
        ...
        overloads: {
            "UpdateRegion()": "updateRegion",
            "ComputeWorldVertices(Slot, float[], int, int)": "computeWorldVertices"
        }
    });
